...
pub async fn get_by_id(pool: &MySqlPool, id: &str) -> Result<Option<User>, sqlx::Error> {
    let row_opt = sqlx::query(
        "SELECT id, username, password, role, admin_id, location_id FROM users WHERE id=? AND delete_time IS NULL",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_by_username(pool: &MySqlPool, username: &str) -> Result<Option<User>, sqlx::Error> {
    let row_opt = sqlx::query(
        "SELECT id, username, password, role, admin_id, location_id FROM users WHERE username=? AND delete_time IS NULL",
    )
    .bind(username)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_all(pool: &MySqlPool) -> Result<Vec<User>, sqlx::Error> {
    let rows = sqlx::query("SELECT id, username, password, role, admin_id, location_id FROM users WHERE delete_time IS NULL")
        .fetch_all(pool)
        .await?;
    Ok(rows
        .into_iter()
...
pub async fn delete(pool: &MySqlPool, id: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE users SET delete_time=NOW() WHERE id=?").bind(id).execute(pool).await?;
    Ok(())
}

pub async fn count(pool: &MySqlPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query("SELECT COUNT(*) as c FROM users WHERE delete_time IS NULL").fetch_one(pool).await?;
    Ok(row.get::<i64, _>("c"))
}
...
pub async fn ensure_default_admin(pool: &MySqlPool, username: &str, password: &str) -> Result<(), sqlx::Error> {
    if get_by_username(pool, username).await?.is_none() {
        let hashed = crate::security::crypto::hash_password(password).map_err(|_| sqlx::Error::Protocol("hash".into()))?;
        let user = User {
