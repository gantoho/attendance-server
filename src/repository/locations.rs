...
pub async fn get_by_id(pool: &MySqlPool, id: &str) -> Result<Option<Location>, sqlx::Error> {
    let row_opt = sqlx::query(
        "SELECT id, name, latitude, longitude, radius, admin_id FROM locations WHERE id=? AND delete_time IS NULL",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_all(pool: &MySqlPool) -> Result<Vec<Location>, sqlx::Error> {
    let rows = sqlx::query("SELECT id, name, latitude, longitude, radius, admin_id FROM locations WHERE delete_time IS NULL")
        .fetch_all(pool)
        .await?;
    Ok(rows
        .into_iter()
...
pub async fn delete(pool: &MySqlPool, id: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE locations SET delete_time=NOW() WHERE id=?").bind(id).execute(pool).await?;
    Ok(())
}

pub async fn count(pool: &MySqlPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query("SELECT COUNT(*) as c FROM locations WHERE delete_time IS NULL").fetch_one(pool).await?;
    Ok(row.get::<i64, _>("c"))
}
