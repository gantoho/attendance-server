...
pub async fn get_all(pool: &MySqlPool) -> Result<Vec<AttendanceRecord>, sqlx::Error> {
    let rows = sqlx::query("SELECT id, user_id, location_id, latitude, longitude, timestamp, status, error_message FROM records")
        .fetch_all(pool)
        .await?;
    Ok(rows
        .into_iter()
...
pub async fn get_by_user(pool: &MySqlPool, user_id: &str) -> Result<Vec<AttendanceRecord>, sqlx::Error> {
    let rows = sqlx::query("SELECT id, user_id, location_id, latitude, longitude, timestamp, status, error_message FROM records WHERE user_id=?")
        .bind(user_id)
        .fetch_all(pool)
        .await?;
    Ok(rows
...
pub async fn count(pool: &MySqlPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query("SELECT COUNT(*) as c FROM records").fetch_one(pool).await?;
    Ok(row.get::<i64, _>("c"))
}
